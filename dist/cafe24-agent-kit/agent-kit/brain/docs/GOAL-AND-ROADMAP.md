...
| **2. 입력 강화** ✅ | 레퍼런스 **URL→토큰 자동**(P4, extract-tokens.mjs) + 히어로/배경 **이미지 추출**(P3, extract-assets.mjs) + input-pipeline.md | 파이프라인 front door | **실증 완료 2026-06-22** (ecudemo392574: 토큰·에셋 추출 작동, P3·P4 해소). 보정거리: text-sub 극단값·Figma 안정화 |
| **3. spine 관통** 🔄 | 레퍼런스 → 입력추출 → 캡처 → 3축 게이트 → (수정) → 재검증 | end-to-end 1회 증명 | **진단까지 관통 성공** (입력추출·캡처·3축게이트 실증, override 결함 적발). **수정은 base 이탈로 롤백** → skin1 에서 EZ-on-legacy 재설계로 이관(메인 1면부터). SPINE-VALIDATION.md |
