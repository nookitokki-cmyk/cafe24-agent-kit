...
| ID | 문제 | 위치 | 판정 |
|----|------|------|------|
| A-4a | `00_시작하기/` 없는데 `CLAUDE.md`가 링크 | `agent-kit/CLAUDE.md` L26 | ❌ 깨진 참조 |
...
| ❌ 키트 내부 불일치 | **2** | AGENT 578줄·F전체 한 파일 / `02_막혔을때/함정-INDEX` 없었음 → **이번에 INDEX 생성** |

상세 한 줄 목록: [`02_막혔을때/함정-INDEX.md`](../../02_막혔을때/함정-INDEX.md)

### C-3. 키트 구조 (문법 감사 부수 결과)

| ID | 문제 | 판정 |
|----|------|------|
| C-3a | F1~F26이 `CAFE24-SMARTDESIGN-AGENT.md` 한 파일에만 — 진입 시 과부하 | ⚠️ `02_막혔을때/함정-INDEX.md` 추가(Phase 1-C) · Phase 2에서 CLAUDE 링크 |
...
| 1-A | FTP·몰ID | §A | ✅ |
| 1-B | OAuth | §B | ✅ |
| 1-C | HTML·F함정 | §C, 02_막혔을때/함정-INDEX | ✅ |
| 1-D | Easy vs HTML | §D | ✅ |
| 1-E | skin 매핑 | §E | ✅ |
...
| 파일 | 내용 |
|------|------|
| `brain/docs/VERIFICATION-EVIDENCE.md` | smoke 5/5·skin 매핑 라이브 (2026-06-19) |
| `brain/_evidence/phase-1a-api-quotes.txt` | Admin API (FTP·Themes·OAuth) |
| `brain/_evidence/phase-1b-api-quotes.txt` | OAuth·앱 등록 |
| `brain/_evidence/phase-1c-api-quotes.txt` | 스마트디자인 basic·module 목록 인용 |
| `brain/_evidence/phase-1d-api-quotes.txt` | editor_type·HTML 편집창·Easy 미수집 목록 |
| `brain/_evidence/phase-1e-api-quotes.txt` | skin_no/skin_code·themes·shops·실측 매핑표 |
| `brain/_evidence/p1d_admin_api.html` | Admin API 전체 스크랩 (재검증) |
| `brain/_evidence/sds1d_design_front_smart_sdsupport_editor.html` | 편집창 가이드 원본 |
| `02_막혔을때/함정-INDEX.md` | F1~F26 한 줄 인덱스 + 출처 등급 |
| `brain/_evidence/dev_*.html` | Phase 1-B scrape 원본 (재검증용) |
| `../MANIFEST.txt` | 복제 정책 |
| (원본 참고) `키트-배포판/00_시작하기/00-아무것도-모를-때.md` | FTP 분기 초안 (공식 대조 전) |
