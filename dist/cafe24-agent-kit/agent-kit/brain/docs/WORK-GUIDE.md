...
> 구매템플릿(skin2) 분석을 통해 도출한 방법론을 정리한 것.
...
|---|---|---|
| 대표 | skin2 (구매템플릿) | skin10 |
| HTML 자유 제작 | **가능** (헤더/푸터/배너 직접 작성) | 제약 많음 (`data-ez-*` 틀에 갇힘) |
| 카테고리 GNB | `module="Layout_category"` | `data-ez-module="menu-main/1"` |
| 우리 작업 적합도 | ✅ 적합 | ❌ 부적합 |

### 결론 2 — 구매템플릿(skin2)의 정체 = "EZ 테마를 걷어내고 _ext 독립 레이어로 재구축" (심층 분석 확정)

> 상세 근거: `web/cafe24/clients/template-02/구매템플릿-ANALYSIS.md` (CSS·HTML·JS 3영역 심층 분석)

- 구매템플릿는 카페24 **"아키테이블" EZ 테마**를 베이스로 가져왔다 (CSS 계보가 EZ 테마와 동일 — `layout.css:1` `아키테이블_style` 주석, `.ez-align`·CSS변수 등)
...
- 헤더·푸터·메인을 `_ext/inc/`로 외부화 + `ext.js`(jQuery 설정 주입 엔진, EZST와 무관) 독자 시스템
- **즉 "구매템플릿는 EZ를 걷어냈다"가 정답.** (단순 구형도, EZ 활용도 아닌 = EZ 출신 + 완전 비활성화 + 독립 재구축)

### 결론 3 — skin15(순수 구형)는 구매템플릿와 다른 계보라 베이스 부적합

- skin15는 구형 XHTML·`@media` 0개(고정폭)·`main.css` 없음·`min-width:1480px` 구형 마커 = **구매템플릿(아키테이블 EZ)와 완전히 다른 종(種)**
- skin15로 작업하면 구매템플릿의 디자인·반응형을 재현할 수 없음 → **베이스로 쓰지 말 것**

### 결론 4 — 우리 베이스: 구매템플릿와 같은 "아키테이블 EZ 출신"을 깊이 걷어낸다

- **skin14**가 구매템플릿(skin2)와 **같은 아키테이블 EZ 출신** (`layout.css:1` 주석 동일). → 구매템플릿 방식 재현에 적합
- EZ 잔재(body max-width·EZST·ez-settings)를 **구매템플릿 수준으로 마저 걷어내면** skin2와 거의 같아짐
- 우리가 좌측 쏠림·아이콘 빠짐으로 헤맨 이유 = **걷어내기가 구매템플릿보다 얕았기 때문** (EZ 잔재가 함정 유발)

> ⚠️ 아래 본문 중 "skin10"=공식 EZ 스킨, "skin14/skin12"=EZ 걷어낸 작업 베이스, "skin15"=부적합 구형.
> EZ테마를 걷어내 작업할 때는 §15 "EZ 걷어내기" + 잔재 제거(body max-width 등)를 구매템플릿 수준까지 한다.
...
| | 공식 EZ 스킨 (skin10) | 일반 스마트디자인 (skin2 구매템플릿) |
|---|---|---|
...
구매템플릿(skin2)는 이 엔진을 `ez/init.js`로 직접 스킨에 복사해 탑재한 것.
...
EZ테마(`data-ez-*` 속성이 있는 스킨)를 구매템플릿 방식으로 자유롭게 작업하려면,
먼저 **EZ 속성을 걷어내** 일반 스마트디자인으로 되돌린다. → **15장 참고**
(구매템플릿가 base(EZ 정품테마)를 가져와서 했던 방식과 동일)
...
구매템플릿 원본은 `.inner`를 페이지별로 쪼개서 선언한다.
이 방식은 **서브 페이지에서 max-width가 안 먹히는 함정**이 있다.

```css
/* ❌ 구매템플릿 원본 — 페이지별 분산 (서브 페이지 누락) */
...
### ✅ 폭 정합 최종 정답 — skin2(구매템플릿) 방식 그대로 (실측 확정)
...
### 🔑 EZ 걷어낸 스킨 vs 구매템플릿 베이스 — body max-width 차이 (중요)

| | skin2 (구매템플릿 베이스) | EZ 걷어낸 스킨 (skin14 등) |
|---|---|---|
...
(구매템플릿 베이스엔 없던 EZ 특유 단계 — 안 풀면 body째 좌측 쏠림)
...
## 12. 구매템플릿 → NK 리네이밍 규칙 (skin2/구매템플릿 복제 시)

일반 스마트디자인 스킨을 복제하면 구매템플릿 명칭(`_ext`, `ext.js`, `--ext-*`)이 그대로 남는다.
...
|---|---|
| `/_ext/` 폴더 | `/_nk/` |
| `ext.js` | `nk-core.js` (또는 유지) |
| `--ext-theme-color` 등 CSS 변수 | `--nk-theme-color` |
| `EXT['key']` (setup.js) | `NK['key']` |

### 파일명 변경 시 — 참조처 동시 수정 필수

예: `ext.js`를 `nk-core.js`로 바꾸려면 아래 3곳을 **모두** 함께 수정해야 한다.

```bash
# 1. 어디서 참조하는지 먼저 검색
grep -rn "ext.js" --include="*.html"

# 결과 예시 (skin12 기준):
#   _nk/inc/common.html:25       <script src="/_nk/js/ext.js"></script>
#   board/review/read.html:61    <script src="/_nk/js/ext.js"></script>
#   board/review/modify.html:60  <script src="/_nk/js/ext.js"></script>
...
- `_nk/js/ext.js` 가 구매템플릿 명칭으로 남아있음 (참조처 3곳: `_nk/inc/common.html`, `board/review/read.html`, `board/review/modify.html`)
...
> **언제 쓰나**: EZ테마로 생성한 스킨을 구매템플릿 방식(HTML 직접 제작 + CSS 오버라이드)으로
> 자유롭게 작업하고 싶을 때. EZ 전용 속성을 제거해 일반 스마트디자인으로 되돌린다.
> 구매템플릿가 base(EZ 정품테마)를 가져와 했던 작업과 동일하다.
...
| 파일 | 이유 |
|---|---|
| `ez/ez-module.html` | EZ 관리자 연동 메타데이터 (구매템플릿도 남겨둠) |
...
구매템플릿(skin2)는 이것들이 전부 없다. 제거 대상 4종 (보통 `@js(/js/common.js)` 다음에 몰려 있음):
...
> **ez/ 폴더 자체(init.js, ez-module.html, ez-settings.json)는 삭제 불필요.** 구매템플릿도 ez/ 폴더는
...
> skin14(아키테이블 EZ 걷어낸 베이스)의 메인을 skin2(구매템플릿) 순서를 차용해 럭셔리 재판매 템플릿으로
...
```
사용: `<i class="ph ph-arrow-right"></i>`. 구매템플릿 원본의 FontAwesome(`fa-*`)는 전부 Phosphor로 교체.
...
동시에 한 번에 띄운다(독립 작업):
- **에이전트 A — skin2 토닝 추출**: `skin2/_ext/css/{custom,common}.css` + 구매템플릿 개조 base(`layout/basic/css/sub_style.css`, `ec-base-button/table/ui/box.css`) + `css/module/{order,member,myshop}/*` 정독 → 영역별(버튼·테이블·폼·장바구니·회원·게시판·타이틀) `셀렉터{속성:값}` → 우리토큰 매핑표.
...
### 18-6. ★ 잔존 흰/회색/골드 박멸 = base CSS 직접 토큰화 (skin2/구매템플릿 방식, 2026-06-03 확립)
...
**오버라이드(`#contents *` transparent !important)로 덮지 말고, skin2처럼 base CSS의 하드코딩 색을 우리 토큰 참조로 직접 치환한다.** 토큰 정의는 custom.css `:root` 에 이미 있음(skin2의 `_ext/color.css` 역할).
...
**⚠️ base CSS 직접 수정 규칙:** 원래 "base 수정 금지"였으나 — 재판매 템플릿(우리 소유, 우리가 업데이트 시점 통제)에서는 skin2/구매템플릿처럼 base 토큰화가 더 우월(오버라이드 해킹·!important 전쟁·캐시싸움 제거). 클라이언트 운영 스킨에서 카페24가 base 를 덮어쓸 위험이 있을 때만 오버라이드 방식 유지.
...
**대전제(검증됨)**: 색·톤=CSS 오버라이드 / **레이아웃·구조=HTML 재작성**(`_nk/inc/`). skin2 가 `@import(/_ext/inc/header·footer.html)` 로 자기 마크업을 쓰는 방식을 차용.
