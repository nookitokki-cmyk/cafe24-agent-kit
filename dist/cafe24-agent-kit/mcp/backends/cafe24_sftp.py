...
class Cafe24SFTP:
    ...
    def _client(self) -> paramiko.SFTPClient:
        ...
        # MITM 방지: 호스트 키 검증 (TOFU — 첫 연결 시 known_hosts 저장, 이후 매번 검증)
        _remote_key = t.get_remote_server_key()
        _known_path = os.path.expanduser("~/.ssh/known_hosts")
        _hk = paramiko.HostKeys()
        if os.path.exists(_known_path):
            _hk.load(_known_path)
        _saved = _hk.lookup(self.cfg["host"])
        if _saved is None:
            os.makedirs(os.path.dirname(_known_path), exist_ok=True)
            _hk.add(self.cfg["host"], _remote_key.get_name(), _remote_key)
            _hk.save(_known_path)
        elif _saved.get(_remote_key.get_name()) != _remote_key:
            raise paramiko.SSHException(
                f"SFTP 호스트 키 불일치: {self.cfg['host']} — "
                "서버가 교체됐거나 MITM 공격 가능성. "
                f"~/.ssh/known_hosts 에서 {self.cfg['host']} 항목을 확인하세요."
            )
        self._transport = t
