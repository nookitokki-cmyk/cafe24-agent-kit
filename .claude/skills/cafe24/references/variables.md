...
> 변수는 **해당 모듈 scope 안에서만** 치환됩니다. 모듈 바깥에서 쓰면 **빈 값**으로 렌더링됩니다(`{$변수명}` 문자 그대로 노출 아님 — [실측 2026-07-06]). ※ 단, 오타·중괄호 누락 시엔 문자 그대로 보일 수 있음.
...
| `{$image_small}` | 작은(축소) 이미지. **목록 마우스오버 롤오버에도 활용**(관리자 '축소 이미지' 칸에 2번째 사진 등록 → `onmouseover="this.src='{$image_small}'"`) | 상세·목록 |
...
헤더·푸터 레이아웃 module 안에서 자주 쓰는 공통 변수(역시 해당 module scope 안에서만 치환).
...
### 변수가 **빈 값**으로 나올 때 (또는 오타로 `{$변수명}` 그대로 보일 때)
