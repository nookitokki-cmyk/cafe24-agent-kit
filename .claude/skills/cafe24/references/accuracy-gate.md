...
## 5. [제안/초안] 4번째 축 — binding (바인딩 보존) · 아직 활성 게이트 아님

> **상태: 제안(draft). 현행 3축 판정에는 미포함.** 재마크업 방법론(`references/module-remarkup.md`) 도입 시 "구조를 새로 짰는데 모듈 데이터 바인딩이 살아있는가"를 자동으로 잡는 **정적 검사 축** 초안. qa-checker 환각과 무관한 **규칙 기반(정적)** 이라 신뢰 가능.

**왜 필요한가**: 재마크업 트랙의 흔한 사고 = `module=` 래퍼를 지우거나, 반복단위를 1개로 줄이거나, `input name`/`{$action_*}`을 빠뜨려 **화면은 멀쩡한데 상품이 1개만 나오거나 결제가 안 되는** 상태. visual 축은 못 잡는다(스크린샷상 그럴듯함).

**검사 항목(재마크업된 `_nk/inc/*` 대상, 정적 grep/AST)**:
```
binding.blocking (하나라도 실패 시 NEEDS_WORK):
  - 원본 대비 사라진 module="..." 래퍼가 있는가
  - product_list* 모듈의 반복단위 id="anchorBoxId_{$...}" 블록이 2개 미만인가
  - 🔴 등급 모듈(module-safety.json)에서 필수 input name / {$action_*} 이 누락됐는가
  - [계층] 반복단위가 목록 모듈 직계인가 (사이에 다른 module= 래퍼가 끼면 바인딩 실패) — §module-remarkup 2-8
```
**판정(제안)**: `binding.blocking == 0` 이어야 PASS.

> **★정적 검사는 "필요조건"일 뿐, "충족조건"이 아니다.** `binding.blocking == 0`(정적 PASS)이어도 실제 바인딩이 되는 건 아니다(모듈 계층 오류 등은 정적으로 못 잡음). **충족조건 = 라이브 렌더에서 `xans-record-`가 실제 주입되고 `anchorBoxId_`에 실제 번호가 채워지는 것.** 재마크업 검증은 **정적(check-binding) + 라이브(xans-record 육안)** 2단으로 하고, 라이브 확인은 base가 아닌 임시/신규 스킨에서. `[실증 2026-07-06: 정적 PASS + 라이브 바인딩 실패 사례]`

> ⚠️ **[검증필요]** "정의 밖 변수" 검사(모듈이 제공하지 않는 `{$변수}` 사용)는 모듈별 제공변수 목록이 있어야 정확(현재 kit엔 회원 모듈만 상세). 그전까진 module 래퍼·반복단위·필수 name/action **3개만 정적 검사**하고, 변수 대조는 사람 검토.
> 근거: `references/module-remarkup.md` §2 · P0 실측 `references/page-module-map.md`(basket 184 바인딩 등).
