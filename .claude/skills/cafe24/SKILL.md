...
description: 카페24 스마트디자인(HTML 방식) + 스마트디자인Easy(비주얼 에디터) 양쪽 개발 가이드. 변수/모듈/지시어 문법, 구매템플릿→NK 템플릿 구조, EZ 속성 시스템, 구버전 스킨에 신버전 스타일 차용법, SFTP 배포 포함. Use when: 카페24 스킨 HTML/CSS/JS 편집, 모듈 커스터마이징, 스마트디자인Easy 블록 편집, SFTP 배포 작업.
---

# 카페24 스마트디자인 개발 가이드

> **⚠️ 이 스킬의 위치 = "문법·모듈·함정 사전"이다. 실제 작업 순서(파이프라인)의 SSoT가 아니다.**
>
> - **작업 순서/파이프라인 SSoT**: 두뇌 문서 `agent-kit/brain/docs/CAFE24-SMARTDESIGN-AGENT.md` + 워크플로우 `agent-kit/01_작업하기/workflows/01~10` (+`05.5-mockup-first`). **충돌 시 두뇌 문서 우선.**
> - **출처**: 이 스킬의 문법·함정은 실제 프로젝트(익명 demo-brand) + 공식 문서(sdsupport.cafe24.com) 기반. 틀린 내용 발견 시 즉시 수정.
> - **⚠️ 예시 주의**: 아래 폴더 구조·스킨 경로·SFTP 전제는 **한 예시 클라(HTML 방식)** 기준이다. 클라마다 다르다 — 현 몰이 **SmartDesignEasy(`/sde_design/base/`) + FTP 배포**일 수 있으니 작업 전 **실제 폴더·배포 수단을 확인**할 것.
>
> **실제 작업 방식의 핵심 기법 5종 (이 문법 사전엔 상세 없음 → 두뇌 문서/워크플로우 참조):**
> 1. **목업-우선 시각 게이트** — 자립형 HTML 목업 → 렌더 → DS grep 게이트 → **승인 후에만** 라이브 (`05.5-mockup-first.md`).
> 2. **CDP 승리규칙 전수조사** — "라이브≠시안"이면 `CSS.getMatchedStylesForNode`로 승리 규칙 출처 파일 특정 → 레거시(대개 우리 옛 커스텀 CSS 후행주입) 제거 → 재실측 (`06-verify-loop.md` · 두뇌 §13/F37).
> 3. **optimizer 캐시 이원** — `@css`/`@js`엔 `?v=` 금지(번들 2~5분 지연) / 직접 `<link>`는 `?v=` 필수. **파일명 아닌 computed·PNG가 증거.**
> 4. **XANS 앵커 우선** — `ec-base-*` 단독 앵커 금지(번짐). `.xans-*` 앵커 + 말단만 `ec-base`. 특이도 전쟁은 `!important` 대신 `:not(.owner)` (두뇌 §5 B8-2).
> 5. **DS 토큰 정본** — 색·폰트·간격은 클라별 `nk-tokens.css`/`root.css` 단일 제어. 값 하드코딩 금지, `var(--nk-*)`.

---

## 작업 전 준비물 (디자인 3종 세트)

> 카페24 작업 결과가 들쭉날쭉한 근본 원인은 **디자인 입력물(준비물)이 없어서**입니다.
> 준비물이 없으면 AI가 색·폰트·간격·컴포넌트를 매번 새로 지어내 몰 전체가 제각각이 됩니다.
> 아래 3종 세트를 먼저 갖춘 뒤 작업을 시작합니다.

| # | 준비물 | 정체 | 비유 | 위치 | 없으면 |
|---|---|---|---|---|---|
| ① | 디자인 토큰 | 색·폰트·간격의 **값** | 물감·재료 | [`design-tokens/`](design-tokens/README.md) · `nk-tokens.css` | 색·폰트를 매번 지어냄 |
| ② | 컴포넌트 갤러리 | 조립된 **완성 부품** | 조립된 가구 | [`component-gallery/`](component-gallery/README.md) · `nk-components.css` | 버튼·카드 모양이 페이지마다 다름 |
| ③ | 브랜드 프로필 | 클라이언트별 정보 | 주문서 | [`brand-profile/`](brand-profile/README.md) | "이 몰 톤이 뭐였죠?" 반복 질문 |

**핵심 지시 패턴:** 실제 작업 시 "이 몰의 `component-gallery`에 있는 부품(`.nk-btn--primary`, `.nk-form-row`, `.nk-card`)을 **그대로 재사용**하고, 값은 `nk-tokens.css` 토큰을 써라. 새로 지어내지 마라."

> 준비물이 어떻게 만들어지는지(레퍼런스/Figma → 토큰 → 갤러리)는 [`references/input-pipeline.md`](references/input-pipeline.md) 참고.

---

## 0. 두 시스템 비교 (작업 전 먼저 확인)

| 구분 | 스마트디자인 (HTML 방식) | 스마트디자인Easy |
|-----|------------------------|-----------------|
| 편집 방식 | HTML/CSS/JS 직접 코딩 + SFTP | 비주얼 블록 에디터 |
| FTP 접근 | 가능 | 불가 (에디터에서만) |
| 커스텀 자유도 | 매우 높음 | 제한적 |
| 스킨 경로 | `/skin1/`, `/mobile/` 등 | `/sde_design/base/` |
| 전용 속성 | `module="모듈명"` | `data-ez-*`, `ez-prop`, `ez-var` 추가 |
| 상호 호환 | ❌ Easy 에디터에서 열 수 없음 | HTML 방식 스킨 편집 불가 |

### 실제 스킨 3개 비교 (demo-brand 프로젝트 기준)

| 항목 | skin1 (구매템플릿) | skin2 ★ 작업 베이스 | skin5 (경량 참고) |
|------|-------------------|-------------------|-----------------|
| 파일 수 | 777 | 612 | 458 |
| 커스텀 레이어 | `_ext/` | `layout/basic/header·footer.html` | `_nk/` |
...
### 구매템플릿/NK 템플릿과 EZ 시스템 관계 (자주 혼동)

구매템플릿/NK 템플릿은 **스마트디자인(HTML 방식)** 기반이며 SmartDesignEasy가 아님.
단, 각 스킨마다 EZ 속성이 포함된 위치가 다름.

```
skin1/
├── layout.html, index.html       → data-ez-* 없음 (표준 SmartDesign)
├── _ext/ 또는 _nk/              → 커스텀 에셋 — 여기서만 작업
...
│   ├── _ext/ 또는 _nk/   ← 커스텀 에셋 (CSS/JS/inc)
│   │   ├── css/
│   │   ├── js/               ← 핵심 JS: ext.js 또는 nk.js
...
### 구매템플릿 → NK 리네이밍 (이 프로젝트의 관례)

| 구버전 (구매템플릿) | 신버전 (NK) |
|--------------|------------|
| `/_ext/` | `/_nk/` |
| `--ext-theme-color` | `--nk-theme-color` |
| `EXT['key']` (JS) | `NK['key']` |
| `ext.js` | `nk.js` |
...
```javascript
// 구버전 (구매템플릿)
EXT['class-popup-ext'] = 'on';          // 팝업 표시
EXT['text-cstime-1'] = '평일 09:00~18:00'; // 고객센터 시간 텍스트
EXT['href-ic-talk'] = 'https://...';     // 카카오톡 링크
...
4. `setup.js` 변수명 교체 (`구매템플릿` → `NK`)
5. HTML 클래스명 `ext-*` → `nk-*` 일괄 교체
...
<!-- ✅ 올바름 — 하드코딩 -->
<span class="nk-drawer__brand">demo-brand</span>
```
...
## 12. 부가 자료 인덱스 — 필요할 때만 읽기

본문은 메인/목록 + 두 시스템 비교 위주. 실무 작업 시 아래 폴더의 자료를 상황별로 참조.
(토큰 절약 위해 전체 로드 말고 필요한 항목만 검색)

### 📚 `references/` — 상세 사전

상품 상세 / 장바구니 / 주문서 / 회원가입 / 게시판 페이지 작업 시 정확한 모듈 ID·변수·문법 확인.
...
**`snippets/components/`** — HTML 컴포넌트 6개 (카페24 변수 통합)
- `nk-header-sticky.html` — 스크롤 시 고정 헤더 (로고·GNB·검색·장바구니 배지)
- `nk-product-card.html` — 상품 카드 (이미지 hover 2번째 + 가격 + 배지)
- `nk-banner-slider.html` — 메인 배너 슬라이더 (자동재생·인디케이터·이전다음)
- `nk-footer-standard.html` — 표준 푸터 (사업자정보·고객센터·SNS)
...
- `nk-button-system.css` — 버튼 (primary·secondary·outline·ghost·danger + sm·md·lg)
- `nk-form-controls.css` — input·textarea·select·checkbox·radio + 에러 상태
- `nk-modal-system.css` — 모달·팝업 (오버레이·닫기 버튼·하단 시트 변형)
- `nk-toast-notification.css` — 토스트 알림 (success·warning·error·info, 자동 사라짐)

**`snippets/js/`** — JavaScript 모듈 6개 (Vanilla, jQuery 의존성 없음)
- `nk-sticky-header.js` — 스크롤 시 고정 헤더 효과
- `nk-product-hover.js` — 마우스오버 2번째 이미지 전환
- `nk-scroll-animation.js` — 스크롤 페이드인 (IntersectionObserver)
- `nk-modal-toggle.js` — 모달 열기/닫기 (ESC + 오버레이 + 포커스 트랩)
...
- `design-tokens/example-tokens.json` — demo-brand 예시
- `design-tokens/builder-guide.md` — `css-builder` 에이전트가 JSON → CSS 변환할 때 따르는 규칙

### 🌐 `module-browser.html` — 시각적 모듈 브라우저

브라우저에서 열면 모든 카페24 모듈이 그림으로 어떻게 생겼는지 볼 수 있는 단일 HTML 카탈로그.
검색 + 카테고리 탭 + 다크모드 + 복사 버튼. 비코더가 "어떤 모듈을 써야 하나" 결정할 때 가장 먼저 보는 자료.
