...
> 작성: 2026-06-09 · 대상 몰: demo000 (template-02) · 상태: 설계 단계
...
- 멀티 클라이언트(reference-case 등) 확장 시: 몰별 `cafe24_config_{mall}.py` + `*.token.json` 분리.
...
- SFTP FTP 비번 **만료/IP 차단** 리스크 상존(reference-case는 이미 만료 이력).
