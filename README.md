...
이미 작업 중인 몰이 여러 개라면 먼저 **`/작업목록`** 으로 어디까지 진행됐는지 확인하세요. 실제 서버 반영 전에는 **`/업로드준비`** 로 audit·백업·승인 게이트를 통과해야 합니다.
...
| 여러 몰 작업 상태 확인 | 채팅 `/작업목록` — `clients/*/.workflow.md` 기준 작업 목록 |
| 실제 업로드 전 안전 준비 | 채팅 `/업로드준비` — audit·manifest·백업·승인 게이트 |
