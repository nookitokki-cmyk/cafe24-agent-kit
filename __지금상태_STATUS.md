...
- [~] Step 4  초보자 친화 재편: [4a ✅] 문서 익명화 + 내부/증거문서 _archive 격리 + 라이브스냅샷 삭제 → 배포본 클라식별자 0(가드 제외) / [4b 🔄] 폴더 재편 ✅(사람용 00_시작하기·01_작업하기·02_막혔을때 ↔ 기계용 brain·connect·.claude, git rename 이력 보존) + 깨진 링크 ✅(19곳→0) + 클라누출 추가제거 ✅(SLOWAGINGS·paransky) + MCP코드(server.py·kit_tools.py)·셸스크립트 새 구조 반영 ✅ + verify-kit ALL PASS / **남은 4b** = 중복 진입점 6→1 통합(편집) + "getting-started" 단어잔재 21곳 / [4c ⬜] 입문자료 신규
- [ ] Step 5~6 (계획 파일 참조)

## 다음 할 일
Step 4b 남은 것: **중복 진입점 6→1 통합**(루트 README·agent-kit README·00_시작하기/README·키트-시작-가이드·00-아무것도-모를-때·이-키트는-누구-용인가 → 단일 진입점 + 나머지 한 줄 리다이렉트) + 본문 "getting-started" 단어잔재 21곳 정리. → 그다음 4c(용어집·0-이키트가뭔가요·첫수정·실패복구 신규 작성).
