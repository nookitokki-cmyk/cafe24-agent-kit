...
상세: `workflows/05-reference-intake.md` · 시안 제출: `getting-started/시안-제출-체크리스트.md`  
**관리자·개발자센터 메뉴 안내 전:** `rules/cafe24-admin-verify.md` (웹 검색 필수)
...
→ **예**이면: **동일 base 템플릿** + `@media` (별도 mobile 스킨 아님). 검증 viewport 390은 **메인 URL** (`/m/` 아님). `rules/responsive-mobile.md` 준수.
...
타입 가이드 (`workflows/05-reference-intake.md` §타입):
...
**사용자 「예」 후:** `clients/{몰}/reference-intake.md` 저장 권장 → `/디자인수정` 또는 `workflows/03-reference-renewal.md` 3단계부터 → Phase별 `workflows/06-verify-loop.md`.
