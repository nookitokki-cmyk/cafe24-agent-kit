...
- **`{$변수}`는 `module="..."` 안에서만 치환된다.** module 밖에 쓰면 **빈 값**으로 렌더링됨(텍스트 노출 아님 — [실측 2026-07-06]) → module 밖 텍스트는 하드코딩.
- **module 영역 구조를 임의 변경 금지** — 바인딩이 깨진다. 스타일만 바꾸거나 module 바깥에 래퍼 추가.
- **지시어**(전부 괄호 단일태그): `<!--@layout(/경로)-->` · `<!--@import(/경로)-->` · `<!--@css(/경로)-->` · `<!--@js(/경로)-->` · `<!--@contents-->`(괄호 없음). 주석 형식이라 linter가 경로 오타를 못 잡으니 육안 확인 필수. (쌍태그·name속성·`@module`은 가짜 — SKILL.md §1)
  - **`@css`는 `?query`를 못 받는다** (붙이면 CSS가 통째로 드롭됨 — [실측]). 캐시 무력화는 페이지 URL에 `?v=N` 으로.
