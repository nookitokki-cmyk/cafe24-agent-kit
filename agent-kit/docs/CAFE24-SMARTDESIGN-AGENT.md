...
>
> **IDIO(skin2)의 정체**: 카페24 "아키테이블" EZ 테마를 베이스로 가져와 **EZ 기능을 전부 걷어내고**(`ez-settings.json`·EZST·`data-ez-*`·body max-width 제거) `_idio/` 독립 레이어로 재구축한 것. "ez 폴더 있다 ≠ EZ 쓴다"(로드 0건이면 죽은 껍데기). 우리 베이스(skin14)도 같은 아키테이블 EZ 출신 → IDIO 수준까지 걷어내면 skin2와 거의 같아진다.
...
```
사용: `<i class="ph ph-arrow-right" aria-hidden="true"></i>`. 다른 아이콘 라이브러리 혼용 금지. IDIO 원본의 FontAwesome(`fa-*`)는 전부 Phosphor로 교체.
...
**D3. base CSS 2레이어 토큰화** — ① `layout/basic/css/*.css` ② **`css/module/**/*.css`(188개)** 둘 다. (skin2/IDIO도 module을 781개 토큰화함.) 한 레이어만 하면 `.totalPrice`·`.tabProduct`·골드가 영원히 잔존.
...
| 파일명만 리네이밍·참조처 미수정 | 경로 깨짐(`grep -rn "idio.js" --include="*.html"`로 참조처 먼저 검색) |
...
35. **EZ GUI ↔ HTML 충돌 (F35)** — Easy **타입** 디자인 + FTP/HTML 대량 수정 시 섹션 GUI 메타와 소스 불일치 → 초기화 오류. **FTP 주력 EZ-on-legacy**는 HTML 타입 skin + EZ **코드** 선별 이식 (`workflows/07-ez-on-legacy-setup.md` Phase 0-D). EZ 마크업 in HTML skin ≠ Easy 타입 등록. 상세: `docs/common-pitfalls.md` §F35.
36. **EZ 이식·제거 함정** — (1) Easy/EZ 파일을 HTML skin에 **통째 덮어쓰기** → 스마트배너·EZST·module 불일치. (2) `ez-settings.json`·`@js(/ez/init.js)` **무분별 삭제** → 레이아웃 옵션·런타임 붕괴 — 역할 확인 후 선별. (3) **전량 제거**는 사용자 명시 HTML 전환 시만 — `strip_ez.py` (STEP 2, `WORK-GUIDE.md` §15). `data-ez-*`는 header/footer 옵션용으로 **남기는** 패턴도 있음(IDIO/아키테이블).
...
- ❌ **함정(실패 사례)**: `.inner_t/.info_left/.info_right`에 `display:flex; gap`을 강제했더니 **base 2단 푸터 레이아웃과 충돌해 구조 전부 깨짐**(기본정보 전폭 늘어짐·우측블록 아래로). IDIO 푸터 HTML은 내부 중첩이 복잡해 flex로 덮으면 흐트러진다.
...
skin2(IDIO)는 회원 페이지를 **중앙 narrow 박스 + 카페24 기본 박스 벗기기 + 세로 폼 + 하단 버튼 균등분할**로 커스텀. 우리는 같은 방식 + 우리 톤(그레이지·차콜·클린 인풋).
```css
...
**skin2/IDIO 실증**: `layout/basic/layout.html`이 base 헤더/푸터를 안 쓰고
```html
<!--@import(/_idio/inc/header.html)-->   <!-- IDIO 자체 클린 헤더 마크업 -->
<!--@import(/_idio/inc/footer.html)-->   <!-- IDIO 자체 클린 푸터 마크업 -->
```
→ `_idio/inc/header.html`·`footer.html`에 **자기 마크업을 직접 작성**(원하는 컬럼·정렬 구조)하고 `_idio/css/`로 스타일. base 카페24 구조에 갇히지 않는다.
...
- IDIO→NK 리네이밍 시 파일명+참조처(`grep -rn "idio.js" --include="*.html"`) 함께 교체.
