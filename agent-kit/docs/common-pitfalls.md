...
**필수 규칙:** [`rules/ez-contents-width.md`](../rules/ez-contents-width.md) — **`#container #contents` + `width:100% !important`**. 스니펫: [`docs/snippets/ez-contents-override.css`](snippets/ez-contents-override.css).
...
- [ ] **`#container #contents` width override** — override CSS 첫 블록 (`rules/ez-contents-width.md`)
...
- [ ] **필수 규칙:** [`rules/ez-contents-width.md`](../rules/ez-contents-width.md)
- [ ] override CSS **첫 블록:** `#container #contents { width:100% !important; margin:0 !important; }`
- [ ] 메인 + PLP + (해당 시) PDP/narrow 타입별 블록 (`docs/snippets/ez-contents-override.css`)
...
- `rules/ez-contents-width.md` — mandatory rule
- `docs/snippets/ez-contents-override.css` — paste-ready
...
- [ ] `rules/responsive-mobile.md` — MO = base `@media`, upload base only (mobile sync는 예외)
...
스니펫: [`docs/snippets/ec-paginate-override.css`](snippets/ec-paginate-override.css)
...
- **FTP·에이전트 작업:** 관리자 **HTML 타입** skin 복사본 + EZ 마크업 선별 이식 — [`workflows/07-ez-on-legacy-setup.md`](../workflows/07-ez-on-legacy-setup.md) **Phase 0-D**
...
- **권장:** EZ base는 **`layout+필수 module+css` 선별 이식** — [`workflows/08-ez-three-step-pingpong.md`](../workflows/08-ez-three-step-pingpong.md) Phase B · [`07-ez-on-legacy-setup.md`](../workflows/07-ez-on-legacy-setup.md)
- **커스텀 레이어:** `_ref{id}/` 만 수정 · EZ core·`skin1` 읽기 전용
- **전량 제거:** 사용자가 **HTML 전환**을 명시했을 때만 — [`WORK-GUIDE.md` §15](WORK-GUIDE.md) + `strip_ez.py` (미리보기 → `--write`)
- **삭제 전 확인:** `ez/ez-module.html`·`smart-banner/init/` 등 시스템 파일은 유지 ([WORK-GUIDE §15](WORK-GUIDE.md) 표)
- **검증:** `data-ez` 잔여 0 + 카페24 코어 `module=` 생존 · Pre-flight C1 PASS ([`06-verify-loop.md`](../workflows/06-verify-loop.md))
...
- [`CAFE24-SMARTDESIGN-AGENT.md`](CAFE24-SMARTDESIGN-AGENT.md) §6 F36
- [`WORK-GUIDE.md`](WORK-GUIDE.md) §15 (`strip_ez.py`)
- [`workflows/08-ez-three-step-pingpong.md`](../workflows/08-ez-three-step-pingpong.md) Phase B·C
