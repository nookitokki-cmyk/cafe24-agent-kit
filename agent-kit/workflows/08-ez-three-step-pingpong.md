...
- Phase C **strip** 후 HTML-clean DOM이면 에이전트·FTP 편집·F27 회피에 유리 — **키트 기본** ([`EZ-STRATEGY.md`](../docs/EZ-STRATEGY.md)).
...
| strip **없이** `data-ez-*`만 유지한 채 장기 커스텀 | F27·detail.css·에이전트 markup 노이즈 — **파일럿 스킵**만 예외 ([`EZ-STRATEGY.md`](../docs/EZ-STRATEGY.md) §4) |
...
| 필요 요소 | 키트 내 위치 |
|-----------|-------------|
| 워크플로우·함정 | `workflows/07`, `06`, 본 문서 `08` |
| EZ 걷어내기 | `docs/WORK-GUIDE.md` §15, `strip_ez.py` |
| MCP SFTP/API | `cafe24_sftp_*`, `cafe24_list_themes` 등 |
| 검증 | `06-verify-loop.md`, `work/scripts/ref*-score-*.py` |
| 부트스트랩 (선택) | `get_kit_guides` — [`HYBRID-ARCHITECTURE-DRAFT.md`](../docs/HYBRID-ARCHITECTURE-DRAFT.md) Phase 1 |
...
2. `_ez-backup/` 백업
3. `strip_ez.py` 미리보기 → 대상 파일 `--write` ([`WORK-GUIDE` §15](../docs/WORK-GUIDE.md))
...
> → [`clients/ecudemo400786/.workflow.md`](../clients/ecudemo400786/.workflow.md) · [`EZ-STRATEGY.md` §4](../docs/EZ-STRATEGY.md)
