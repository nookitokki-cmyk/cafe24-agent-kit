...
1. 레퍼런스 인입 (코드 금지)
   └─ 페이지 인벤토리 → 페이지 타입 표 → 실측 시트 → 사용자 「예」
      (workflows/05-reference-intake.md)
...
2. **`/`** 와 **서브 1페이지**(PLP 권장)에서:
   ...
   ```
3. **FAIL이면:** [`rules/ez-contents-width.md`](../rules/ez-contents-width.md) + [`docs/snippets/ez-contents-override.css`](../docs/snippets/ez-contents-override.css) 적용 후 재측정. Phase 1+ 진행 금지.
...
1. **사용자에게 관리자 설정 확인 요청** — `rules/responsive-mobile.md` §필수: 관리자 설정 경로를 붙여넣어 안내 (MCP·FTP 변경 불가)
...
- **구현:** `@media (max-width: 1023px)` on `/sde_design/base/` — mobile 전용 HTML/CSS 신규 작성 금지 (`rules/responsive-mobile.md`).
...
- 함정: `docs/common-pitfalls.md`
- **EZ #contents:** `rules/ez-contents-width.md`
- 키트 로드맵: `docs/kit-roadmap.md`
