...
> ★ **이 문서가 키트의 단일 진입점입니다.** 처음이라면 여기서 시작해서 아래 **폴더 맵 → `00_시작하기/`** 순서대로 따라가면 됩니다. 다른 안내 문서는 모두 이 문서를 가리킵니다.
...
전체: [`commands/COMMANDS.md`](commands/COMMANDS.md) · 매칭 가이드: [`00_시작하기/OMC-명령어-매칭가이드.md`](00_시작하기/OMC-명령어-매칭가이드.md)
...
| `.claude/skills/cafe24/recipes/` ✅ | **모듈 조합 레시피북 7개** (v2.4.0 신규, 안정) |
| `.claude/skills/cafe24/templates/` 🧪 | 페이지 타입 스타터 5개 (v2.4.0 실험적) |
| `.claude/skills/cafe24/snippets/` 🧪 | HTML 6 + CSS 8 + JS 6 = 20개 (v2.4.0 실험적) |
...
- **`recipes/01~07.md`** — 모듈 조합 레시피북 ("어떤 화면 → 어떤 모듈" 매핑 사전)
...
- `snippets/components/` — HTML 컴포넌트 6개 (header·card·slider·footer·breadcrumb·quick-view)
