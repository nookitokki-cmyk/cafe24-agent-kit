...
5. **막히면** → [`02_막혔을때/F-상황-인덱스.md`](02_막혔을때/F-상황-인덱스.md) (**F1~F36** 증상·Q&A·프롬프트 한곳)  
...
| 명령 | 용도 |
|------|------|
| `/작업목록` | `clients/*/.workflow.md` 를 읽어 여러 몰 작업 상태와 다음 액션을 요약 |
| `/업로드준비` | 실제 SFTP 반영 전 `skin-audit`·manifest·백업·승인 게이트 |

이 두 명령은 기존 명령을 대체하지 않습니다. 기존 작업 흐름은 아래 워크플로와 `00_시작하기/키트-시작-가이드.md` 의 콤보를 그대로 따릅니다.
...
| `01_작업하기/` | 워크플로 (인입·실측·production-skin-safety·verify-loop) |
| `02_막혔을때/` | F1~F36 함정·인덱스·postmortem |
...
| **`clients/_verified-template/`** | **검증 템플릿** — 데모몰 2곳 QA 통과한 브랜드 중립 스킨 32파일(`src/`) + 설치 안내(README)·케이스 스터디 |
| **`02_막혔을때/F-상황-인덱스.md`** | **F1~F36 허브** (증상·Q&A·프롬프트·postmortem) |
| `00_시작하기/QnA-쉬운말로.md` | **[F?]** 태그 Q&A 18개 |
| `00_시작하기/실측-격차-설명가이드.md` | 실측 격차 이유·대응 |
| `00_시작하기/프롬프트-템플릿.md` | F코드 붙은 복붙 프롬프트 12종 |
| `02_막혔을때/함정-INDEX.md` | F1~F36 한 줄 인덱스 |
| `01_작업하기/workflows/` | 01~10 (인입·실측·배포 전 안전 점검·verify-loop 포함) |
...
SFTP 업로드 전에 로컬 스킨 폴더를 read-only로 점검합니다.

```bash
python mcp/cli.py skin-audit agent-kit/clients/_verified-template/src --json-out tmp/skin-audit.json
```

- 결과: `summary`, `criteria`, `reference_edges`, `blockers`, `warnings`
- 금지: 업로드, 삭제, 원격 변경, 접근 불가 파일을 빈 파일로 생성
- 자세한 절차: [`01_작업하기/workflows/10-production-skin-safety.md`](01_작업하기/workflows/10-production-skin-safety.md)

---

## 검증 템플릿 사용법

새 몰을 빈 스킨에서 시작하지 마세요 — 실제 데모몰 2곳에서 QA 게이트를 전부 통과한 **브랜드 중립 스킨 템플릿**이 키트에 들어 있습니다.

| 무엇 | 어디 |
|------|------|
| 템플릿 본체 (32파일) | `clients/_verified-template/src/` |
| 설치 안내 (비코더용 — FTP 순서·토큰 값 바꾸기·주의사항) | [`clients/_verified-template/README.md`](clients/_verified-template/README.md) |
| 검증 근거 (예시몰 링크·재현성 수치·중립화 요약) | [`clients/_verified-template/CASE-STUDY.md`](clients/_verified-template/CASE-STUDY.md) |

1. `/새클라이언트`로 `clients/{몰ID}/` 폴더를 만들고
2. `_verified-template/src/`를 그 안으로 **복사**해서 작업하세요 — 템플릿 폴더 자체는 kit-update 때 **통째로 덮어써집니다**
3. FTP 업로드·브랜드 적용(토큰 값)은 설치 안내 README 순서대로
