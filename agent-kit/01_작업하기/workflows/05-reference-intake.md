...
Q1~Q6 질문 핑퐁
  ...
  → 페이지 타입 표
  → 실측 시트 (+ HTML 구조 격차)
  → 디자인 시스템 시트 (타이포 스케일 먼저 → 컴포넌트/에셋/유닛 인벤토리 + XANS 앵커)
  → 사용자 「예」
...
### 4-2. 디자인 시스템 시트 (코드 전 게이트 — 요소측정/레퍼런스인입에서 완료)

실측 숫자를 **① 타이포그래피 스케일(먼저 확정)** + **② 컴포넌트·에셋·유닛 인벤토리(각 부품의 XANS 앵커 셀렉터 포함)** 로 정리해 승인받는다. 템플릿·XANS 앵커 찾는 법은 `04-measure-first.md` §디자인 시스템 시트, 방법론은 두뇌 `brain/docs/CAFE24-SMARTDESIGN-AGENT.md` STEP 1·§5 B8-2 참조.

- **타이포 먼저**: h1~sm 스케일 → `--nk-fs-*` 토큰 매핑.
- **컴포넌트는 XANS 앵커로**: 각 재사용 부품(버튼·폼·탭·표·카드·페이지네이션 등)이 어떤 module→`.xans-*`로 렌더되는지 개발자도구로 확인해 인벤토리에 기록. 코드는 이 앵커에 스타일을 얹는 것으로 시작(`ec-base-*` 단독 앵커 금지 — 번짐).
...
| 파일 | 내용 |
|------|------|
| `clients/{몰}/reference-intake.md` | 인벤토리 + 타입표 + 실측 + **디자인 시스템 시트(타이포 스케일 + 컴포넌트/에셋/유닛 인벤토리 + XANS 앵커)** + HTML 격차 |
