...
for f in CLAUDE.md README.md \
         docs/CAFE24-SMARTDESIGN-AGENT.md docs/common-pitfalls.md docs/F-상황-인덱스.md \
         traps/INDEX.md workflows/07-ez-on-legacy-setup.md; do
  [[ -f "$AK/$f" ]] && pass "doc $f" || fail "missing agent-kit/$f"
done

# 2) 초보자 동선(getting-started) 최소 5개
gs=$(find "$AK/getting-started" -maxdepth 1 -name '*.md' -type f 2>/dev/null | wc -l | tr -d ' ')
[[ "$gs" -ge 5 ]] && pass "getting-started ${gs}개" || fail "getting-started 부족(${gs})"
...
sec=$(find "$KIT" -type f \
        \( -name 'sftp_*.json' -o -name '*.token.json' -o -name 'cafe24_config_*.py' \) \
        -not -path '*/.git/*' -not -path '*/node_modules/*' 2>/dev/null | wc -l | tr -d ' ')
if [[ "$sec" == "0" ]]; then pass "시크릿 파일 0개"
else fail "시크릿 파일 ${sec}개 발견 (배포 금지)"
  find "$KIT" -type f \( -name 'sftp_*.json' -o -name '*.token.json' -o -name 'cafe24_config_*.py' \) -not -path '*/.git/*' >&2
fi
...
if command -v python >/dev/null 2>&1; then
  if (cd "$KIT/mcp" && python -c "import server" >/dev/null 2>&1); then
